...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t * BoardMade = malloc(sizeof(*BoardMade));
  BoardMade->width = w;
  BoardMade->height = h;
  BoardMade->totalMines = numMines;
  BoardMade->board = malloc(h * sizeof(*BoardMade->board));
  for (int y = 0; y < h; y++){    
    BoardMade->board[y] = malloc(w * sizeof(*BoardMade->board[y])); 
    for(int x = 0; x < w; x++){         
       BoardMade->board[y][x] = UNKNOWN;
    }
  }
 
  for(int i = 0; i < numMines; i++){    
      addRandomMine(BoardMade);
  }
